...
const newClient= (): Client => ({
  ...
  export const useClientStore = defineStore("client", () => {
    ...
    const createNewClient= () => {currentClient
    ...
    const saveClient = async () => {
      ...
      if (!currentClient.value.cedula) {
        ...
        mesageBox.value = {type:'success',message:`el cliente ${currentClient.value.nombre} ha sido creada correctamente`};
      } else {
        await updateApi(currentClient.value.cedula, currentClient.value)
        mesageBox.value = {type:'success', message:`el cliente ${currentClient.value.nombre} se ha actualizado correctamente`};
      }
      await loadClient();
      dialog.closeDialog();
